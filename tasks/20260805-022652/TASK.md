...
`tatr overwrite <task-id> [-t [+|-]<tags> ...] [-p [+|-]<priority>] [title]`
Given a task-huid, you can modify its tags, priority and title.
...
`tatr overwrite <huid> -p 55 -t -bug` which will overwrite the task's priority setting it to 55 and remove bug to its tags
